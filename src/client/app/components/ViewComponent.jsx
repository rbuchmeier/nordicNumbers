...
var ViewComponent = React.createClass({
  render: function() {
    if (this.props.race.racers.length > 0) {
        var first_racer_time = this.props.race.racers[0].time;
    };
    return (
      ...
      <div className="row" key="label"><em>
            ...
            {this.props.race.racers.map(function(racer, i) {
                var percent_back = (racer.time - first_racer_time) / first_racer_time * 100;
                return (
		   ...
		   <div className="col-md-1">{racer.grade}</div>
                   ...
                   <div className="col-md-2">{percent_back.toFixed(1)} %</div>
                    </div>
                );
            })}
      </div>
